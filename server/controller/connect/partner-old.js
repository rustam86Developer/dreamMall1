...
module.exports.addPartner = async function (req, res) {
    ...
    try {
        ...
        //if partner type isv and isvType self
        let partnerInfo = {};
        if (req.body.partner.isvPaymentMode && req.body.partner.isvPaymentMode == constants.isvPaymentMode.SELF) {
            partnerInfo = await dao.addPartner(req.body, transaction)
        }
        else {

            let paymentInfo = await paymentHelper.addToVault(req.body.cardDetail, req.body.paymentPayload);

            if (paymentInfo.response_code == constants.payment.SUCCESS) {
                req.body.paymentInfo.vaultId = paymentInfo.customer_vault_id;
                partnerInfo = await dao.addPartner(req.body, transaction)
            } else {
...
module.exports.uploadDocument = async function (req, res) {
    try {
        ...
        let remote =   CONFIG.PARTNER_DOCUMENT +  req.body.partnerId +'/'+ req.files[0].originalname;
