(function($) {
    ...
    var methods = {
        ...
        restoreSelection: function(text, mode) {
            //Function to restore the text selection range from the editorFpl
            var node;
        ...
        imageWidget: function() {
            ...
            var container = $('<div/>', { 'class': "inner-modal-height" });
            var navTabs = $('<ul/>', {
                class: "nav nav-tabs mb-2"
            }).append($('<li/>', {
                class: "nav-item"
            }).append($('<a/>', {
                class: "nav-link tabs-text active",
                "href": "#uploadImageBar_" + _idSuffix,
                "data-toggle": "tab"
            }).html("From Device"))).append($('<li/>', {
                class: "nav-item"
            }).append($('<a/>', {
                class: "nav-link tabs-text",
                    ...
                    reader.onload = (function(imageFile) {
                        return function(e) {
                            ...
                            var image = $('<img/>', {
                                ...
                                console.log('#imageList_' + _idSuffix)
            ...
            var imageURL = $('<input/>', {
                type: "url",
                class: 'stikkum-form-control border-bottom-grey',
                id: "imageURL_" + _idSuffix,
                placeholder: "Enter URL"
            }).appendTo(getImageURL);
            var getURL = $("<button/>", {
                class: "Communications-btns mt-2",
                type: "button"
        ...
        tableWidget: function(mode) {
            ...
            var tblCntr = $('<div/>', { //Outer Container Div
                ...
                id: "tblRows" + idExtn,
                type: "text",
                class: "stikkum-form-control border-bottom-grey form-control-width",
                value: 2
                ...
                class: "col-xs-12 col-sm-6 col-md-6 col-lg-6"
            }).append($('<label/>', { for: "tblAlign" + idExtn, text: "Alignment" })).append($('<select/>', { id: "tblAlign" + idExtn, class: "custom-select stikkum-form-control border-bottom-grey place-holder dropdown form-control-width" }).append($('<option/>', { text: "Choose", value: "" })).append($('<option/>', { text: "Left", value: "left" })).append($('<option/>', { text: "Center", value: "center" })).append($('<option/>', { text: "Right", value: "right" }))).append($('<label/>', { for: "tblBorder" + idExtn, text: "Border size" })).append($('<input/>', {
                id: "tblBorder" + idExtn,
                type: "text",
                class: "stikkum-form-control border-bottom-grey form-control-width",
                value: 1
        ...
        init: function(options) {
            ...
            var menuItems = {
                ...
                'insert_link': {
                    ...
                    "modalBody": $('<div/>', {
                            ...
                            class: "stikkum-form-control border-bottom-grey form-control-link ",
                ...
                'placeHolders': {
                    ...
                    "custom": function(button) {
                        methods.restoreIESelection.apply(this);
                        var flag = 0;
                        var splCharDiv = $('<div/>', { id: "placeholder", class: "placeholdercls", css: { "display": "none" } }).click(function(event) { event.stopPropagation(); });
                        var splCharUi = $('<ul />', { id: "special_ui" });
                        var editor_Content = this;
                        if ($(this).data("editor").data("plcharsBtn")) {
                            flag = 1;
                            $(this).data("editor").data("plcharsBtn", null);
                        } else
                            $(this).data("editor").data("plcharsBtn", 1);

                        if (flag == 0) {
                            for (var i = 0; i < placeholders.length; i++) {
                                splCharUi.append($('<li />').html(placeholders[i].text).attr('title', placeholders[i].name).mousedown(function(event) { event.preventDefault(); }).click(function(event) {
                                    if (navigator.userAgent.match(/MSIE/i) || navigator.userAgent.match(/Windows NT.*Trident\//)) {
                                        var specCharHtml = $(this).html();
                                        methods.insertTextAtSelection.apply(this, [specCharHtml, 'html']);
                                    } else {
                                    ...
                                    $('#placeholder').remove();
                                    $(editor_Content).data("editor").data("plcharsBtn", null);
                                }));
                            }
                            splCharUi.prependTo(splCharDiv);
                            splCharDiv.insertAfter(button)
                ...
                'attachment': {
                    ...
                    "commandname": null,
                    "custom": function(button) {
                        console.log("FUNCTION TO ATTACH VIDEOS");
            ...
            var menuGroups = {
                ...
                'extraeffects': ['strikeout', 'hr_line', 'splchars', 'placeHolders', 'camera', 'attachment'],
            ...
            $(document.body).mousedown(function(event) {
                ...
                if (!target.parents().addBack().is('#placeholder') && (target.closest('a').html() != '<i class="fa fa-asterisk"></i>')) { //Clicked outside
            ...
            /**
             * Condition for select Placeholder color change. 
             */
        ...
        createModal: function(modalId, modalHeader, modalBody, onSave) {
            ...
            var modalElement = $('<div/>', {
                ...
                class: "modal-content modal-shadow"
            }).append($('<div>', {
                class: "modal-header modal-header-cust"
            }).append($('<button/>', {
                type: "button",
                class: "close",
                "data-dismiss": "modal",
